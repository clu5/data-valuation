...
def main(checkpoint_path, data_root, debug):
    ...
    loader = lambda ds: data.DataLoader(
        ds, batch_size=16, num_workers=8, pin_memory=True, shuffle=False
    )
    ...
    lr = 1e-4
    fit_kwargs = dict(
        criterion=criterion, epochs=20 if not debug else 2, debug=debug, patience=5
    )
    ...
    model_bought = train.fit(
        model_bought,
        loader(bought_data),
        val_loader=loader(id_test_data),
        optimizer=opt_bought,
        **fit_kwargs,
    )
    model_random = train.fit(
        model_random,
        loader(random_data),
        val_loader=loader(id_test_data),
        optimizer=opt_random,
        **fit_kwargs,
    )
    id_bought_scores, id_bought_labels = train.get_scores(
        model_bought, loader(id_test_data)
    )
    id_random_scores, id_random_labels = train.get_scores(
        model_random, loader(id_test_data)
    )
    ...
    od_bought_scores, od_bought_labels = train.get_scores(
        model_bought, loader(od_test_data)
    )
    od_random_scores, od_random_labels = train.get_scores(
        model_random, loader(od_test_data)
    )
