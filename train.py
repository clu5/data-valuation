...
def main(dataset, arch, save_dir, pretrain, epochs, lr, debug):
    ...
    data_splits = {split: dataset.get_subset(split, transform=_transforms) for split in splits}
    ...
    for split, dataset in data_splits.items(): print(split, f'{len(dataset)}')
    ...
    loader_splits = {split: get_loader(split)('standard', dataset, **loader_params) for split, dataset in data_splits.items()}
    ...
    model = fit(model, loader_splits['train'], criterion, optimizer, epochs=epochs, val_loader=loader_splits['id_val'], debug=debug)
    ...
    }, Path(save_dir) / f'{dataset}-{arch}-{epochs}-{"pretrain" if pretrain else ""}.pt')
